...
app.post(
    ...
    async (c) => {
        ...
        // 1. 게시글 데이터 가져오기
        const { data: post, error: postError } = (await supabase.from("posts").select("*").eq("id", postId).single()) as any;
        ...
        try {
            ...
            const completion = await groq.chat.completions.create({
                model,
                temperature: 0.7,
                max_tokens: 1500, // <think> 태그 공간 확보 및 짤림 방지
                messages: [
                    {
                        role: "system",
                        content: `당신은 지친 마음의 본질을 비춰주는 대한민국 최고의 상담 전문가 '프리즘'입니다. 
...
- **절제된 통찰**: 과한 비유나 '우주'와 같은 모호한 표현은 피하고, '본질', '여정', '울림', '평온' 같은 단어를 사용하여 깊은 여운을 남기세요.
